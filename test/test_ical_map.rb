...
class IcalMapTest < Test::Unit::TestCase
  def test_read_calendars_from_file
    assert ((IcalMap::IcalUtils.get_calendars_from_file "test/TerrificTerrariums.ics").first.class.eql? Icalendar::Calendar), "The object read should be a Calendar but is not one"
  end
  ...
  def test_assign_multiple_calendar_events_from_file
    p IcalMap::IcalUtils.get_events_from_file "test/multipleEventsCalendar.ics"
    assert ((IcalMap::IcalUtils.get_events_from_file "test/multipleEventsCalendar.ics").first.class.eql? IcalMap::Event), "The object read should be a Event but is not one"
  end

  def test_lat_lng_are_retrieved_from_location
    event = (IcalMap::IcalUtils.get_events_from_file "test/TerrificTerrariums.ics").first
